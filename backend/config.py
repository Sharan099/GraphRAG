...
# Load environment variables from backend/.env (if present) before reading them.
try:
...
# ── LLM API (answers) — Groq / Qwen ─────────────────────────────────────────────
# Answers are generated by Qwen 3 32B served on Groq's free, OpenAI-compatible API.
#   qwen/qwen3-32b  → fast, strong reasoning, free tier  ← default
GROQ_MODEL         = os.getenv("GROQ_MODEL", "qwen/qwen3-32b")
LLM_MODEL          = GROQ_MODEL    # display name used by the API/UI
LLM_MAX_TOKENS     = 800
LLM_TEMPERATURE    = 0.0
# Network resilience: per-request timeout (seconds) and automatic retries on
# timeout / connection / 5xx errors. The Groq SDK retries with backoff.
LLM_TIMEOUT        = float(os.getenv("LLM_TIMEOUT", "120"))
LLM_MAX_RETRIES    = int(os.getenv("LLM_MAX_RETRIES", "3"))

# ── RAGAS evaluation judge (offline eval only) ──────────────────────────────────
# RAGAS metrics need an LLM "judge". We deliberately use a fast, non-reasoning
# Groq model here (separate from the answer model) so structured-output scoring is
# quick and reliable — Qwen3's chain-of-thought makes RAGAS calls very slow.
RAGAS_JUDGE_MODEL  = os.getenv("RAGAS_JUDGE_MODEL", "llama-3.3-70b-versatile")
RAGAS_JUDGE_MAX_TOKENS = int(os.getenv("RAGAS_JUDGE_MAX_TOKENS", "8000"))
